...
<div id="if_cmnt_content">
	<ul class="main_board">
		<c:forEach var="i" begin="0" end="${fn:length(bbsList)-1}" step="1">
			<c:choose>
				<c:when test="${i%2==0}">
				...
				<strong><c:out value="${bbsList[i].bbsNm}" />&nbsp;</strong>
				<a href="javascript:fn_egov_loadArticleList('<c:out value="${bbsList[i].bbsId}" />','');" class="more"><spring:message code="comCopCmy.communityUseMgrBaseTmpl.more"/></a> <!-- 더보기 -->
				<c:if test="${fn:length(articleList[i]) == 0}">
				<ul class="list null">
				...
				<c:if test="${fn:length(articleList[i]) > 0}">
				<ul class="list">
				<c:forEach var="article" items="${articleList[i]}" varStatus="status">
				<li>
