...
//		String tmplatCours = cmmntyService.selectCmmntyTemplat(cmmntyVO);
    ...
    public String selectCmmntyMainContents(@ModelAttribute("searchVO") CommunityVO cmmntyVO, ModelMap model) throws Exception {
		...
		System.out.println("BBSMasterService : " + bbsResult.size());
		...
		for (int i = 0; i < bbsResult.size() && i < bbsResult.size(); i++) {
		    ...
		    boardVo.setPageUnit(bbsResult.size());
		    boardVo.setPageSize(bbsResult.size());
	
		    boardVo.setFirstIndex(0);
		    boardVo.setRecordCountPerPage(bbsResult.size());
