...
public class EgovLoginController {
	 ...
	 * @exception Exception
	 */
	@RequestMapping(value = "/uat/uia/actionFirn.do")
	public String actionFirn(ModelMap model) throws Exception {
	...
	public String actionLogin(@ModelAttribute("loginVO") LoginVO loginVO, HttpServletRequest request, ModelMap model) throws Exception {
		...
		// 2. 로그인 처리
		//System.out.println("로그인 하러 들어간다.");
		LoginVO resultVO = loginService.actionLogin(loginVO);
		//System.out.println("로그인 스비스 끝");
		// 3. 일반 로그인 처리
		if (resultVO != null && resultVO.getId() != null && !resultVO.getId().equals("")) {
			//LOGGER.debug("\n\n 여기여기 " + resultVO.getUserSe() + "\n\n");
			// 3-1. 로그인 정보를 세션에 저장
	...
	public String actionMain(ModelMap model) throws Exception {
		...
		LOGGER.debug("여기여기 actionMain" + user.getUserSe());
		/*
		// 2. 메뉴조회
